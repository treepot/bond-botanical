...
/*if($_SESSION['page_type']!=""){
	header('Location: http://bond-botanical.jp/cs/'.$_SESSION['page_type'].'/');
}else{
	header('Location: http://bond-botanical.jp/cs/');
};*/
...
//購入者へのメール
if(mb_send_mail($_SESSION['email'], $_SESSION['name'].'様、お問い合わせありがとうございます',$message,$add_header) != 0){
...
if(mb_send_mail($_SESSION['email'], $name.'様、お問い合わせありがとうございます',$message,$add_header) != 0){
...
▽注文書・受領書：
http://bond-botanical.jp/cs/pdf/?id=0&format=1{$encode_detail}

▽納品書：
http://bond-botanical.jp/cs/pdf/?id=0&format=2{$encode_detail}
HTML;
