...
$add_header .= "Bcc: bond.web.management@gmail.com";
...
$message = mb_convert_encoding($message,"SJIS-win");
//件名のみWindows live対応
$contact_mail_subject = "=?iso-2022-jp?B?".base64_encode(mb_convert_encoding('【bois de gui】'.$_SESSION['name'].'様、お問い合わせありがとうございます',"JIS","UTF-8"))."?=";
//購入者へのメール
//$contact_mail_title= '【bois de gui】'.$_SESSION['name'].'様、お問い合わせありがとうございます';
if(mail($_SESSION['email'], $contact_mail_subject, $message,$add_header) != 0){
...
$message2 = mb_convert_encoding($message2,"SJIS-win");
$contact_mail_subject2 = "=?iso-2022-jp?B?".base64_encode(mb_convert_encoding($_SESSION['name'].'様からお問い合わせがありました',"JIS","UTF-8"))."?=";
//$_SESSION['name'] = $_SESSION['name'].'様からお問い合わせがありました';
//管理者への確認用
if (mail($_POST['shopAdr'],$contact_mail_subject2,$message2,$add_header) != 0){
};
...
$message = mb_convert_encoding($message,"SJIS-win");
$order_mail_subject = "=?iso-2022-jp?B?".base64_encode(mb_convert_encoding('【bois de gui】'.$name.'様、ご注文ありがとうございます',"JIS","UTF-8"))."?=";

if(mail($_SESSION['email'], $order_mail_subject,$message,$add_header) != 0){
...
▽注文書・受領書・納品書：
https://bond-botanical.jp/pdf/?id=0&format=3{$encode_detail}
HTML;
...
$message2 = mb_convert_encoding($message2,"SJIS-win");
$order_mail_subject2 = "=?iso-2022-jp?B?".base64_encode(mb_convert_encoding('【Order Sheet】'.$name.'様からご注文です',"JIS","UTF-8"))."?=";
mail($_POST['shopAdr'],$order_mail_subject2,$message2,$add_header);
