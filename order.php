...
<section class="lineup">
	...
	<?php
	if($season_display == 1 && $season_open == 0){
		$promo_class = ' close';
	}else{
		$promo_class = '';
	}
	?>
	<?php if($season_display == 1){ ?>
  <h2 class="seasons_ttl"><img src="<?php echo ROOT ?>img/season/ttl_mothers_day.png" alt="2018 Mother's Day (母の日)"></h2>
  <a class="seasons_prod<?php echo $promo_class ?>" href="<?php
	if($season_open == 1){
		echo ROOT.'order/arrangement_mothers_day/';
	}else{
  ...
  --><a class="seasons_prod<?php echo $promo_class ?>" href="<?php
	if($season_open == 1){
		echo ROOT.'order/bouquet_mothers_day/';
	}else{
		echo 'javascript:void(0)';
	}
	?>">
